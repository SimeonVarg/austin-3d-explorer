...
- Originals: `<projects>/austin-reference-images/_owner-phone/originals/IMG_99xx.HEIC`
...
### 1.2 Web photos (local copies under `<projects>/austin-reference-images/_night/`)
...
middle of nautical twilight the row is defined as. The `blue` row beside it is bound to `torre`, which
`sources.json` itself hedges "twilight / blue hour" and which also has no clock.

Looked at side by side, **they are not simply swapped**: `torre` still has a legible ground, a warm
western band and a pale zenith, and `dobie` has silhouetted horizon buildings against a deep blue
zenith, so `torre` is the earlier frame and the file's blue-then-twilight ordering is the right way
round. What is missing is not the ordering but the anchor — **neither end of it is tied to a sun
elevation by anything but a judgement about pixels.** That is enough to compose a sheet against and not
enough to set a threshold from, which is why A2 in the plan carries the same caveat. Settling it takes
one clocked twilight photograph, which this corpus does not contain.
