...
> **CORRECTED 2026-08-27 by the ship lane. The table that stood here was wrong,
> and the conclusion it carried — "not a snapping-tolerance bug", therefore
> nothing to do — was wrong with it.** The numbers below are a fresh brute-force
> recomputation: `data/walk_graph.json` decoded from scratch (delta-coded `n.x`/
> `n.y`, cumulative `e.a`, `e.b` offset from `e.a`), components by union-find over
> all 13,751 edges, then every island node measured against all 10,744 main-network
> nodes by haversine with no spatial shortcut. That decode reproduces every OTHER
> anchor in this section exactly — 219,041 m, 13,751 edges, 47 islands, 5,647 m,
> largest island 1,824.8 m, 10,744 main nodes — which is why the distances, and
> not the decode, are the thing that was broken.

```
within  2 m of the main network:   0 islands
within  5 m:                        4 islands,   913 m
within 10 m:                       12 islands, 1,324 m
within 20 m:                       26 islands, 4,362 m
within 50 m:                       38 islands, 4,670 m
the largest island: 1,825 m of walkway, 19.00 m from the main network
the closest island:  175 m of walkway,  2.45 m from the main network
```
**[M]** So the honest read is the opposite of the one this section used to give.
**Four islands are within five metres**, the nearest at 2.45 m, and the single
largest island — 1,825 m, a third of all unreachable surveyed walkway — sits
**19 m** out, not 118 m. That is well inside tolerances this same codebase
already trusts elsewhere (`DOOR_LINK_MAX_M` 30 m, `UT_VIRTUAL_SNAP` 75 m), and
`bake_walk.py` already owns a snap stage (63 candidates, 8 accepted, components
59 → 48) whose candidate radius simply never reaches these 47 components.

Part of the original claim survives: the long tail really is unmapped
connection — a walk that ends at an unmapped crossing, steps nobody has drawn, a
service drive — and inventing geometry to bridge those is still the thing this
project does not do. But "upstream OSM's problem, nothing to do here" is not
supported by the measurement. Pointing the existing snap stage at these
components is a real, bounded job, and it is written into `QUEUE.md` as one.
