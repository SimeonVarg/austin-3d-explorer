...
- **A dense schedule truncates with an ellipsis**: `Q V 310, 31884 – Wh…`,
  `TAQ 320, 60217 – Quin…`. The course code survives; the instructor does not.
  Never try to recover a truncated name.
- **Unique numbers appear** in the dense variant: `HCF 208B, 51840 – R…`,
  `DRV 355, 29663 – Halloran`. A five-digit number after the course code is a
...
- **An ASYNC / OTHER row sits below the grid** with classes that have *no time
  and no day at all*: `PYX 312L, 71058 – Halloran`, `VNH 317L, 49771 – Vance`.
