# Walk feature â€” progress log

## Where this stands â€” 2026-08-24, thirty seconds
...
â€” the app sent you **569 metres for a walk that is 271**, more than double,
...
**Which of them beat the bar blind? One did â€” the interface.** This pass was
...
- **The interface â€” judged blind, and won.** Real Citymapper walking cards were
...
- **The door and the pavement â€” checked by someone else and confirmed, but
...
- **The stairs â€” the only verdict on record is a loss.** Round 1 was judged and
  failed. Rounds 2 through 8 were never judged at all.
- **The lighting â€” never judged, at any round.**
...
- **The ruler itself had gone out of true.** The tool that measures the wasted
  ...
  measuring the wrong thing on fifteen of nineteen trips. Fixed â€” it now reads
  the real settings off the running page, and its self-check is exact to the
  centimetre on every trip.

**Would I switch it on for everyone? Not yet â€” but it is close.** The routing is
...
accessibility** â€” "step-free", "no mapped streetlight along this route". Getting
...
## 2026-08-24 â€” critic pass, round 3 on the schedule-import "parser" piece: oursWins = false, because there is still no "ours" â€” nothing changed since round 1
...
it is byte-identical to `origin/main` â€” `git diff main acer/si-parser` shows
...
"ics", and "import" turns up zero PRs about a schedule importer â€” the "import"
...
two recon docs â€” nothing else, anywhere, has ever touched this piece.
...
`false` and nothing in `js/wayfind.js` changed at all â€” not a partial attempt,
not a stub, not a dead-end branch. **oursWins = false** â€” same reason as
...
question:** nothing has changed since round 1's gap â€” a builder still needs to
...
## 2026-08-24 â€” critic pass, round 1 on the schedule-import "parser" piece: oursWins = false, because there is no "ours" to look at
...
machine (11 of them) â€” none is named or contains anything for a parser piece.
...
(167 lines) and `docs/import-bar-ut.md` (268 lines) â€” real-format writeups for
...
**oursWins = false** â€” not a loss on merits, a piece that was never built.
...
brief â€” a Google Calendar `.ics` export parser, an Apple Calendar
...
tuple array around `js/wayfind.js:3609` â€” using the two recon docs that
...
## 2026-08-23 â€” the lighting claim, checked at 43 places instead of six: it holds, and it turned up a live oak sitting on top of a street lamp
...
routes, drops 43 sample points along them â€” some where the app says there is a
light, some where it says there is none â€” flies to every one at night, and
...
mapped streetlight and the night frame showed nothing at all â€” because there is
...
about â€” nearly a third of them are under a tree. So the card now says it: "24
mapped streetlights along this route Â· 4 of them are under tree cover."
...
60 random building-to-building walks â€” and it changed nothing on any of the 12
...
at about a third of them â€” the little rings we draw at each counted lamp come
...
streetlights to replace our guesswork â€” the city's whole GIS catalogue, the
...
version with all the numbers is `docs/walk-lit.md` Â§17-24.

## 2026-08-23 â€” recon only, no code touched: UT publishes its own official front-door data, and our heuristic is currently wrong most of the time
...
buildings, 264 (89%) have zero OSM-sourced door data at all â€” every door on
...
`Celebrated_Entrances` layer â€” UT Facilities' own hand-surveyed record of the
...
placed right, just labelled "secondary" â€” so the router (which only ever
...
no `lit` tags, confirmed â€” but the app already has 532 OSM-sourced street
...
barrier-free door sits on another wall â€” which UT's own data proves happens
...
## 2026-08-23 â€” the walk interface, rebuilt for a phone: and the first thing found was that the last phone fix had been silently switched off by a comment
...
fixed once already â€” and the fix had stopped working, because the long comment
...
actually plan around. The strip is deliberately flat â€” it is not a hill profile
...
nothing when you are standing still â€” it only recomputes while the camera moves.
...
any of the three recording modes â€” the gate now walks every element the feature
...
## 2026-08-23 â€” walk UI, round 2 (Acer, `acer/w-ui`)

The walking view had an arrow that argued with its own words. It pointed at
where the next turn was, relative to whichever way you were facing â€” which is
...
instead of hidden behind the little chevron. The camera never moves on its own â€”
that is deliberate â€” so the route the bar is describing can be completely off
...
tooltips do not exist on a phone â€” there is a one-line key now, and the stairs
...
changed nothing, and the mirror-image version â€” pad the bottom instead, which
reads just as sensible â€” would have hidden half of the two longest routes. That
...
## 2026-08-23 â€” the walk now lists itself, step by step
...
walk written out in order. Open the details now and you get it â€” start, walk
33 m, turn right, walk 470 m, turn left, walk 24 m, Welch Hall, north side â€”
...
with "then a sharp right" before you had moved a metre â€” that was the little
straight line we draw from the door to the pavement, not a corner anybody has
ever turned â€” and on the way to DKR it listed twelve turns in 580 m, which is
...
gained a fact â€” the direction of the turn after the next one â€” and it is the
...
## 2026-08-23 â€” the walk interface, round 4: there was no button that put you on the pavement, so half the feature was unreachable

Lane `acer/w-ui`. The bar has always had two faces â€” the answer you read
...
camera standing in a wall by lifting it onto the roof â€” so the button walks the
...
is the second button now, and **Clear** is an âœ•.

Two other things. The little bar showing where the stairs and crossings fall
along the walk **was reading as a slider** â€” a thin track with a bright round
...
was carrying **two complete trip summaries stacked** â€” the second one counting
...
of the lower half of the frame â€” measured by hiding the route, re-photographing,
...
## 2026-08-24 â€” round 1 critic verdict on `acer/w-ui`: oursWins = NO
...
descendants visible â€” clean) and re-ran `scripts/verify/walkmeter.mjs` against
this branch's own served app: 19/20 pairs measured, self-check held at 0.00 m
drift on every pair, 795 m total extra walking â€” same figure round `w-baseline`
...
every 500 ms after tapping **Walk it** on `JES â†’ WEL` (an ordinary 530 m
...
silently ejected back to 900 m â€” the exact altitude `Show route` uses â€” within
...
before ejection also render broken â€” a band of magenta and a flat olive-green
...
`window.__fly.eye().alt` every 500 ms â€” two frames cited,
...
route / âœ•) in one row of the closed bar and stacks current-turn + remaining
ETA + destination note + next-step preview + two buttons into a single
persistent top card while walking â€” denser and more capable (no-stairs and
...
dropping to walking height) â€” so on at least one ordinary real route the
...
Branch left as found â€” no files the builder owns were touched. Ran from a
...
## 2026-08-23 â€” the stairs: one was drawn nowhere, and "avoid stairs" was walking people over them

Went after the stairs. Three things were wrong. First, the city was drawing 188
of the 189 staircases OpenStreetMap has mapped on campus â€” the missing one, on
...
the path to each door â€” and on four buildings (the Computation Center,
...
refuses to arrive at a door it cannot reach without steps â€” it costs 32 more
...
hands it back with what it costs â€” and when there genuinely is no way round
...
`docs/walk-stairs.md` for whoever owns the card â€” four lanes are in that file
this round and I stayed out of it. Pictures in `shots/walk/stairs/`.
Branch `acer/w-stairs`.

## 2026-08-23 (later) â€” the stairs, part two: the card now shows them, and the way round is a button with the price on it

Part one put all the facts on the route and left the interface for whoever owned
the card. Nobody did, so this is that â€” plus four things that only turned up
...
it is beside ("620 m in Â· near WEL"). It says "up the steps" or "down the steps"
where OpenStreetMap has recorded which way is up, and plainly says it does not
know on the rest â€” that is one flight in eight today, and it is a bug in the
graph bake rather than missing data, so the exact patch is written down.

The way round is now a button with its cost already worked out â€” "Step-free:
14â€“19 min Â· 1.2 km / Avoids all 7 sets Â· 250 m further" â€” and when there is no
...
â†’ Tower and then answered "no route that avoids mapped stairs" when you pressed
...
front one â€” which is what UT's own accessible-entrance data says is right, and
...
## 2026-08-23 â€” critic pass on `acer/w-stairs`, round 1: the two checkable claims held, the third couldn't be checked

Independent critic, fresh context, own scripts (not the builder's â€” those live
...
(189, one of them â€” `147362093` â€” tagged `area=yes`) and cross-checked every
...
one of those 84 came back clean on a fresh request â€” 0 that still crossed a
...
button, and the headline actually became "No stairs on this route" â€” and
clicking the "Avoid stairs" checkbox on an open card left it open, both fixes
holding under a real click. Screenshots (desktop + a 393Ã—852 phone frame)
...
leg list against how Citymapper states steps in its own â€” I have no device
...
leg-list format, so `docs/walk-stairs.md`'s own comparison table (Â§R10) is
...
**oursWins: false** â€” not because anything I could test failed (nothing did),
...
and diff our card against it directly â€” until that exists, "matches
Citymapper's format" is asserted, not verified, and shouldn't be treated as
closed.

## 2026-08-23 (round three) â€” the stairs read like a transit app now, because we finally went and looked at one
...
do, then the name of the thing you do it at â€” "in 25 m / Turn right onto /
Goldsmith's Row" â€” with the leg's own size on the right. A third picture, from
...
the building instead of its code â€” "in 630 m / Steps at Robert A. Welch Hall"
where it used to say "Up the steps Â· 620 m in Â· near WEL". Every one of 210
...
Step-Free Entrance") â€” what we checked is that the walk has no steps on it, not
...
near-identical rows in a row â€” three real staircases, but one thing you
actually do â€” so they're one row now that says "3 sets of steps". Anything
...
about eighty pixels off the bottom.** It's one CSS line â€” the card is pinned at
...
lane, so the exact two lines are written down in `docs/walk-stairs.md` Â§R15
...
old switch on made the check *withhold* every bad route, which is it working â€”
...
real buttons. `WAYFIND.on` is still false â€” none of this is public yet.
Pictures in `shots/walk/stairs/` (`r3-*`). Branch `acer/w-stairs`.

## 2026-08-23 â€” the "avoid stairs" answer was walking people down steps, and the check meant to catch it couldn't see them
...
against two different files instead â€” the raw OpenStreetMap survey, and the
staircases the map actually draws â€” and the claim did not hold up. Out of 122
...
straight lines we draw ourselves â€” from the pavement to your starting door, and
...
at all, when the map draws it three metres wide. So the worst case â€” the last
stretch to the door running straight down a flight of steps â€” was the one case
...
flipped â€” the dashed last stretch coming out of the middle of a lit staircase
while the card cheerfully says "No stairs on this route Â· no further to walk
...
is honest rather than good â€” those are buildings whose only doors can be
...
one-line change is in `docs/walk-stairs.md` Â§R23. The whole test harness is now
...
## 2026-08-23 â€” the route now knows which of it has a streetlight on it, and shows you (branch `acer/w-lit`)

The city already draws 236 lights â€” 193 OpenStreetMap street lamps and 43 of
UT's blue-light emergency phones â€” and the walking router could not see a single
...
of those rings at night and the lamp post is standing in it â€” that was the test,
...
safety â€” that would be steering by who bothered to map a lamp, not by where the
...
lit" anywhere â€” every sentence is about what the map holds, and it says out loud
...
never dropping the furniture it thinks it is dropping â€” written up, not touched,
it's another lane's file. Full write-up and the pictures: `docs/walk-lit.md` and
`shots/walk/lit/`. `WAYFIND.on` untouched.


## 2026-08-23 â€” the walk now knows where people said it was dark, not just where a lamp is mapped (`acer/w-lit`, round 2)
...
looking for more. So this pass went looking â€” and found something better than
...
West Campus â€” where you walk home at one in the morning â€” is exactly where the
...
**It still does not send you the long way round on its own** â€” that argument
...
alternative went from 17 to 25 â€” eight routes that now have an option they did
not have â€” at exactly the same median cost of 24 extra metres. Nothing anywhere
...
lamp (563 m â€” the San Gabriel one) and the pin nearest one (9 m, where somebody
wrote "too dim"). At the first, the frame is black â€” no pole, no light, nothing.
...
the words are careful â€” that is in `js/night.js`, another lane's file, so it is
...
with the phone number. Both are in `docs/walk-lit.md` Â§16. Pictures and the full
argument: `docs/walk-lit.md` Â§9-Â§16 and `shots/walk/lit/wc-*.png`. `WAYFIND.on`
untouched.

## 2026-08-23 â€” lit lane, round 4: the lighting box was unreadable, so it got a picture
...
home into West Campus â€” the exact walk this whole thing exists for â€” the
...
your *door* â€” same sentence before, completely different walk. Under it the
...
everywhere it is cool there is nothing at all â€” not one lamp pixel at any of the
...
â€” the easy half. I sampled the hard half, 25 to 60m, at 18 places: **half the
time you can see a streetlight standing there anyway.** The card isn't lying â€”
it counts lamps within 25m of your path â€” but it is right in a way that would
get it called wrong. Rather than widen the radius (which would inflate every
number in this feature), a route with no streetlight at all now adds "Â· 2 more
...
time. The thing it does *not* fix is written down â€” a single dark stretch 28m
...
## 2026-08-24 â€” streetlights, round 5 (`acer/w-lit`)

**Last round left one thing hanging and I went and measured it.** The bar in the
lighting box is amber where a streetlight is mapped and cool where none is â€” but
...
to say "2 more are mapped within 50 m of it" â€” so it was pointing at lamps
...
oak sitting on top of the lamp** â€” hide the trees and 4,000 pixels of lamplight
...
on the ground at the foot of every streetlight we count â€” it's the receipt, so
...
â€” same ring, same shape, just darker, so it reads as "counted, but don't expect
...
of its amber marks â€” same as on the laptop â€” and it never claims more light than
...
space either side. Photo and numbers are in `docs/walk-lit.md` Â§38 for the lane
rebuilding that card.

Routing untouched again â€” every steering number identical to round 3.
Full argument, every number and the three instruments that lied to me first:
`docs/walk-lit.md` Â§32-Â§41.

## 2026-08-23 â€” the walk now ends at the door a student actually uses, because UT publishes which one that is
...
ignoring it â€” scored against that list, the door we walked you to was the right
...
forty doors is now within 15 m of UT's â€” it used to be seven of them. Across all
...
295 m. One got longer on purpose â€” Patterson to Biological Laboratories went up
...
cover, a side door is finally allowed to win when it genuinely saves a walk â€”
...
`data/walk_graph.json` is rebaked by the lane that owns it â€” one command, written
...
## 2026-08-23 â€” round 2 of the door pass: the photographs that were missing, one more building reachable, and the thing that turned out not to work
...
courtyard â€” 700 m to the right door instead of 830 m to the wrong one, with
...
2.5 m out, down from 3.7, and all 56 of them are inside 15 m â€” up from 54 of 55.
...
we tried scored between 27 and 31 metres, and even cheating â€” picking the best
door we hold â€” is still 18 metres out, because for 26 of those 55 buildings the
...
## 2026-08-23 â€” door lane, round 3: the number is checkable now, and it caught a bug
...
own plinth â€” and every way off that plinth is a staircase. Thirty-seven paths
...
of dashed line and gives back three buildings that used to be unreachable â€”
...
sidewalk hunch got a real test â€” the idea that a footpath which dead-ends at a
...
barely says anything about where a door is. The sidewalks themselves are fine â€”
...
## 2026-08-23 â€” the routes were already on the sidewalks; what was crossing the lawns was the last few metres to the door (`acer/w-sidewalks`)

Went in expecting to find the router ignoring the 1,324 sidewalk shapes the
scene draws, and that turned out not to be true â€” measured against every
...
costs. The router will still use one when there is no alternative â€” it has to,
that is how you get out of a building â€” but it will happily walk up to four
...
stopping a stride short of every street corner in the city â€” about 1.4 metres
...
so routes walk around them instead of across â€” that one needs a change in the
graph bake, which is a different lane's file. `WAYFIND.on` untouched.

## 2026-08-23 (later) â€” the campus malls were being drawn as lawns, so the route floated over them

Picking up the same branch after the kerb-apron pass. That pass had already
found the important thing â€” the router was never ignoring the sidewalks, it was
spending invented door links as shortcuts â€” and it noticed in passing that a
...
Forty-four of the pedestrian areas on campus â€” Main Mall, East Mall, the
Speedway courts, the Jester, Gates and Blanton forecourts â€” were being drawn as
...
and eight of the twenty moved â€” PCL to Patton Hall goes 54% to 78%. It costs
...
## 2026-08-24 â€” the baseline meter, built: 20 real class-to-class pairs, a real number
...
student would actually walk back-to-back â€” GDCâ†’JES, WELâ†’PAI, and 18 more,
...
pairs this round â€” the one unmeasurable pair, PHRâ†’BIO, is its own finding:
...
EERâ†’NHB at +298 m (screenshotted, matches the measured number to the metre â€”
...
â€” PMAâ†’MEZ alone would get 231 m worse, because MEZ's two real, front-facing
...
one door, even a UT-verified one, isn't the fix â€” keeping near-tied
...
## 2026-08-24 â€” the walk interface, round 5: the door into the walk had no door out, and the little bar of the journey was still a slider
...
diff â€” the arrow that used to argue with its own words, "Show route" being
...
button left on the bar was the âœ• â€” and the âœ• throws the route away. So the way
...
it** is waiting again. Fixing that turned up a real bug hiding underneath it â€”
...
staircase and every crossing. On a route that has nothing on it â€” which is most
short walks between two neighbouring buildings â€” it is an empty band with one
...
was worked out once, for the whole walk, and never asked again â€” so three
...
just under halfway. And the step-by-step list â€” the one thing this bar has that
looks like every other walking app â€” was behind a bare chevron in the corner
...
reports at that width is not ours â€” it is identical with the whole feature
...
higher â€” so on some bearings the whole route lands behind the answer bar. That
...
## 2026-08-24 â€” w-ui round 6: the walk can keep the camera now, and the bar stopped looking like a segmented control

Picked up round 1's critic verdict on `acer/w-ui` and closed both halves of it.

**The ejection is real and I reproduced it 3 times out of 3** â€” tap `Walk it` on
`JES â†’ WEL` at phone size, the eye drops correctly to 1.70 m, and two and a half
...
with `&fit=1` there is a second one â€” the "frame the route" wait is watching for
...
are the same story â€” I re-shot that pose with every building layer hidden, then
...
forced, because the controller pins the camera's pitch to 84â€“88Â° at walking
...
row â€” `Walk it`, `Show route`, `âœ•` â€” all wearing the same rounded box with the
...
whole feature and it is `Walk it`. Nothing is hidden and no word is lost â€”
...
`docs/walk-ui.md` Â§35â€“40. Branch `acer/w-ui`, port 8815, server killed and port
confirmed free, no scratch scripts left in `scripts/verify`.

## 2026-08-24 â€” the walk bar reads as one journey again (`acer/w-ui`, round 7)
...
**The one thing they have that we don't, we are not allowed to have â€” and that
...
them. Round 5 then deleted that line on every route with nothing on it â€” which
is most short walks â€” and nobody looked at what was left. What was left was two
...
word, nothing new claimed â€” just the two marks we already drew, joined. Before
and after: `shots/walk/ui/r7-bar-before-after.jpg`.

**And one thing on the walking bar was still dressed as a button that isn't
one.** "and then left" â€” the turn after your next turn â€” wore an amber disc in
...
loses the camera roughly one time in five â€” the app's opening flight lands on
...
`r7-walkbar-before-after.jpg`. Full writeup in `docs/walk-ui.md` Â§41â€“46. Branch
...
## 2026-08-24 â€” round 8 critic verdict on `acer/w-ui`: oursWins = true
...
feature at 390x844 in real Chromium â€” not the builder's screenshots.

**Camera fight (the round 1 critic's original finding): held.** Tapped `Walk it`
on JESâ†’WEL and sampled `__fly.eye().alt` once a second for 14 straight seconds.
It read exactly 1.70 m on all 14 samples â€” never ejected. One run isn't the four
the builder logged, but it's a real, fresh, independent check and it agrees.

**Every specific claim in round 7's own writeup, checked, held.** The spine â€”
one hairline from the start tick to the door ring â€” renders correctly; closeup
...
`?autopilot=1`, and `?sliderdemo=1` â€” all three checked, all three pass.
`walkmeter.mjs` against this branch: 19/20 pairs, 0.00 m self-check drift,
795 m total extra â€” identical to `docs/walk-baseline.md`, so nothing upstream
...
navigation at all â€” a pure campus walk there is one line, "12 min Â· Show Map,"
thinner than anything on our bar. So the true bar â€” the GO-pill pre-trip card
and the "19 min / 14:38, turn left onto [street]" live card â€” was pulled instead
...
without another lane.** Its turn card names the actual street â€” "Turn left onto
Rue de l'Ã‰chiquier" â€” twice. Ours never names a path anywhere, not on the live
...
any bar round can close this â€” it's the literal thing Simeon asked for
...
untouched.
## 2026-08-24 â€” "there's no step-free way there" was the one thing we'd never checked, and five times in fourteen it wasn't true

Every round so far checked the walks the app *offers* â€” is this really
...
OpenStreetMap files â€” every staircase deleted â€” and asked it, for each of the
...
the building at all â€” while the real step-free path runs thirteen metres away.
...
and there is one â€” **eleven metres longer**. Screenshots of the card before and
...
genuinely possible on the ground â€” three buildings (Flawn Academic Center, the
...
## 2026-08-24 â€” Stairs, round 6: the walk was step-free and the door at the end of it was not
...
flight â€” and then we deliver somebody to a door nobody ever asked about. UT
...
different door** â€” one of them 63 metres away, round the far side. So the
...
of 38 now, for 353 metres of extra walking spread over 300 routes** â€” about
...
The picture is Perry-CastaÃ±eda Library to the Physics building. Before, the
ribbon stops at the south side. After, on the same camera, it carries on round
to the north-east corner â€” which is the entrance UT lists as barrier-free.
...
publishes have no door of ours within eight metres at all** â€” including Parlin
...
round's checks still green. The stopwatch was thrown out â€” a route proved to do
...
## 2026-08-24 â€” stairs, round 7: the app says "up the steps", and nothing used it

The card has always told you which way a flight goes â€” "up the steps", "down
...
same flight â€” and the two photographs differed by **ten pixels**. Now the
...
shorter, and not one picked up a staircase it didn't have** â€” eight dropped
...
The step-free walk did not move at all â€” 243 of them compared field by field,
...
The price of a climb is the number that was already in the file â€” the same one
...
measuring the city still loading â€” it claimed 193,000 "walk pixels" on a frame
where the real answer is about 9,800 â€” so a noise floor got measured first and
...
**2026-08-24 â€” stairs, round 8 (`acer/w-stairs`).** UT publishes an official
...
half â€” the entrances UT says are *up a flight of steps*, in its own words:
...
start at one of them â€” both from Parlin Hall â€” and the app handed both over with
...
again by a completely different method â€” which is the strongest cross-check
...
Two more things. A question left open since round 5 â€” is the step-free walk
longer than it needs to be? â€” got measured properly and the answer is no: at
...
them they were walking *down* â€” eight walks now print a minute less, and none
print more.
...
## 2026-08-24 â€” streetlights, round 6 (`acer/w-lit`): the third colour, and standing on the pavement instead of hovering over it
...
violet one had been looked at in exactly two places â€” both picked by hand. So
...
person who wrote *"Street light does not work"* â€” and OpenStreetMap has no lamp
...
terrible way to see what a person walking there sees â€” nothing can stand in
...
(`shots/walk/lit/r6-lit-04-eye.png` â€” that's the picture worth looking at).
...
named nowhere on the card â€” and on the walk home into West Campus the bar is
...
and the card explains that behind a tap. I set the bar first â€” if more than a
third of places had real glow with nothing under it, that sentence comes out from
behind the tap â€” then measured it properly and got **3 in 16**, and for the
...
`docs/walk-lit.md` Â§42-Â§51.

## 2026-08-24 â€” lighting (acer/w-lit), round 7
...
one of the sixteen sentences was fine â€” while **three of the marks were not**:
...
it has â€” telling a lit stretch from an unlit one â€” so I framed the bar and framed
...
to check it is actually visible â€” `shots/walk/lit/r7-edge-ab-zoom.png`, and the
...
as a defect â€” a spot the card calls unlit that seemed to have light in it â€” and it
...
marks in two different coordinate systems â€” real, and measured at under one pixel
on 90 routes, so I wrote the bound down and changed nothing.

Routing untouched for the fifth round running. `docs/walk-lit.md` Â§52-Â§58.
## 2026-08-24 â€” the "Avoid stairs" box you could not tick, and one ruler instead of two
...
after the click has finished travelling â€” by which time the card, and the checkbox
...
ninety metres shorter. The panel still opens and closes when you click its own text â€”
...
maps.utexas.edu actually shows: identical, line for line, to the table we ship â€” the
...
## 2026-08-24 â€” critic pass, door round 4: it wins, but avoid-stairs strands a real building
...
server: it reproduced the branch's own claim to the decimal â€” route-length
...
(WCH, MAI, WEL, CAL) â€” every coordinate the app reports off `wayfindUTDoors()`
...
Â· 260 m Â· Stairs: 1 set". After one real click: checked, "1-3 min walk Â· 170 m
Â· No stairs on this route". Click again: unchecked, back to 260 m and Stairs: 1
set. It works, independently confirmed.

**oursWins = true.** Every one of the three bars in the brief â€” extra metres
to the door a student would use, agreement with the door maps.utexas.edu
itself presents, and a checkbox that actually avoids stairs â€” beats what
...
with the box unchecked works fine from three different hubs (GDC, PCL, UTC â€”
790 m / 1.1 km / 1.1 km, 1-2 stair sets). Tick the box on any of those three
and the API returns `{ok:false, why:"noroute"}` â€” and the real UI, driven with
a real click, shows the user "No walking route found" (screenshotted:
the pill card, GDC to CMB, after clicking Avoid stairs for real â€” kept only in
...
step-free component specifically â€” which edge or door is cutting it off from
every hub â€” rather than re-sweeping the global tuning constants, which are
...
## 2026-08-24 â€” the last building "Avoid stairs" refused to take you to
...
H. Jones Communication Center B, and the app said "No walking route found" â€” for
...
to the entrance UT publishes â€” three metres away â€” opens onto a courtyard whose
...
offered if you can actually walk to it. Nothing else changed â€” every one of the
...
UT's own accessibility survey certifies â€” with the last forty metres drawn
...
pieces of route actually got painted before the shutter â€” 83 in the after frame,
...
## 2026-08-24 â€” the door lane, round 6 (`acer/w-door`): one way in became eight

Round 5's critic came back empty, so this round went hunting for its own gap.
It turned out not to be about which door the app walks you to â€” that part has
been right since round 4 â€” but about how you walk up to it. When UT publishes an
...
in the city already gets several â€” the app picks whichever suits your walk â€” and
this was the only kind of door that did not.

It does now. Across every UT building, walked from three different starting
points in both modes â€” 416 trips â€” 127 of them got shorter, five got a few metres
...
while the door you arrive at did not move by so much as a centimetre â€” same
...
and no stairs, ending at the other entrance UT publishes on that building â€” the
one you were always closer to.

The part worth reading twice is what this round refused. Letting the app pick
freely among all the nearby pavement scored much better â€” 68 m of extra walking
instead of 142 â€” but the last few metres to one of these doors is a straight
...
existed, and wrote down the fix â€” it belongs in the entrance bake, not the router.
`WAYFIND.on` still false.

## 2026-08-24 â€” critic pass, door round (after round 6): it still wins, and the "UT door" it's scored against is sometimes the wrong point
...
the decimal â€” route-length extra 795.3 m -> 142.1 m, signed total +209.5 m ->
...
real card (WCH -> Tower) with a real click in between â€” unchecked "3-4 min walk
Â· 250 m Â· Stairs: 1 set", checked "1-3 min walk Â· 160 m Â· No stairs on this
route Â· Avoids 189 mapped staircases." The checkbox genuinely avoids stairs.

**The gap.** UT's own `Celebrated_Entrances_view` carries two different
coordinates per row â€” a `Longitude`/`Latitude` attribute pair and a separate
point `geometry` â€” and this branch (rounds 1-6, `js/wayfind.js` and
...
the point the map actually draws is 30.286023/-97.739757 â€” **18.5 m away**. For
EER, attribute 30.288143/-97.735633 vs geometry 30.288310/-97.735657 â€” **18.75
...
attribute field â€” an internal-consistency check, not a check against what
...
the 20 pairs â€” EER and MAI both appear in the headline pair set (EER->NHB is
...
**oursWins = true anyway.** All three bars in the brief â€” extra metres to the
door a student would use, agreement with the door maps.utexas.edu actually
presents, and a checkbox that actually avoids stairs â€” still land far ahead of
...
design in `docs/walk-door.md` round 6 Â§6 â€” still open, not this round's find.

Server on 8851 killed and port confirmed free. Touched only this file
(`docs/walk-progress.md`) â€” no file `acer/w-door` owns.

## 2026-08-24 â€” door round 7: the critic was right about the coordinate, and fixing it was worth 64 metres

The critic's find held up under a fresh check from scratch. UT's entrance layer
gives every door two positions â€” a pair of ordinary columns and the point the
map actually draws â€” and they are a median 2.7 m apart, fifteen buildings are
...
credit it earns went from 354 m to **405 m**, and â€” the line that matters most,
because it cannot be moved by rewriting the table â€” the independent oracle whose
...
against a referee neither of them controls â€” how far each sits from the wall of
its own building â€” and it is a dead heat: geometry nearer on 18 rows, the columns
...
knocked the building out of the app entirely â€” 0 m from UT's door became 59.9 m,
...
building and every value above 58.8 admits the same set. It now reads 75 â€” chosen
...
## 2026-08-24 â€” critic pass, door round (post-round-7), oursWins=true
...
decimal against the "doors off" baseline (main, no correction) â€” route-length
...
(not through the branch's tooling â€” a raw `curl` against
...
"2-4 min walk Â· 230 m Â· Stairs: 1 set", a visible ribbon descending the Tower's
east side; checked gave `stairSets:0`, "3-5 min walk Â· 270 m Â· No stairs on
...
**The single biggest remaining gap:** the independent pair-file oracle â€”
doors matched to UT's rows by hand, offline, so it can't be moved by rewriting
a table â€” still lands on the wrong door for **11 of its 30 ends (37%)** even
...
from the door itself (`docs/walk-door.md` round 6 Â§6), so the router's last
...
re-measured against the new ones â€” nobody knows today whether that number
...
(`docs/walk-progress.md`) â€” no file `acer/w-door` owns.
## 2026-08-24 â€” the malls were painted but their EDGES were not, and the edge is where the router walks (`acer/w-sidewalks`)

Round 2 finished by saying the last problem â€” the walking line running along the
outline of a mall rather than across it â€” was a routing problem and not
...
line you can walk â€” around the edge. The part that paints the ground drew only
...
So the edge of a mall is now painted as what it is â€” a walk, at the same 2.4 m
...
worse: Winshipâ†’Main goes 71 % to 100 %, RLPâ†’Garrison 71 % to 98 %.

The honest version of the headline is the one about the ribbon rather than its
centreline â€” how much of the drawn strip a person actually sees is off the
paving â€” and that goes **4.2 % to 1.2 %**.
...
put a halo round every mall â€” look at `shots/walk/sidewalks/rim-city-before.png`
...
for the door and graph lanes: `docs/walk-sidewalks.md` Â§12â€“Â§16. `WAYFIND.on`
untouched.

## 2026-08-24 â€” acer/w-sidewalks, round 4: the other half of the question, and 5.75 km of sidewalk nobody can walk on
...
everybody** â€” and every single invented metre, in all forty walks, is the short
...
Then the more interesting question â€” his actual sentence, *"so many sidewalks
are not being utilized properly"* â€” got a number for the first time. Walking all
...
not this lane's file to fix â€” `scripts/bake_walk.py` owns the joining â€” so it is
...
lying on the pavement or hovering over it â€” the exact defect round 2 fixed. So
...
a metre â€” where it turns into a kerb-height wall you cannot miss. The camera can
...
`docs/walk-sidewalks.md` Â§17â€“Â§21. `WAYFIND.on` untouched.

**2026-08-24, later â€” `acer/w-sidewalks`, round 5.** Eight kilometres of real
...
lanes â€” a guess â€” while the sidewalk itself is surveyed. Where the survey puts a
...
to the metre â€” which is the check that says no pavement was invented. Two things
...
`WAYFIND.on` still false. Everything: `docs/walk-sidewalks.md` Â§22â€“Â§26.
## 2026-08-24 â€” `acer/w-sidewalks` round 4 critic: it wins, verified independently

Reviewed `acer/w-sidewalks` (round 4 of 5, HEAD `6c50c1d`) against `origin/main`
as the bar, on the two things the brief actually asks for â€” is the router on
...
--walkaudit` against it â€” the same tool, pointed at the bar instead of taken
on faith. On the harsher "house twenty" fixture: pavement coverage
(`ON A DRAWN WALK`) goes **85.71% â†’ 96.83%**, bare ground **7.52% (695 m) â†’
1.63% (154 m)**, and share of each route running on a real OSM footway
feature rather than an invented line goes **95.38% (mean-per-route 93.07%) â†’
...
opinion here â€” its self-check (browser vs. its own Dijkstra reimplementation)
...
eyeballed at low res, which caught me misreading the corner on first look â€”
zoomed in, the apron is a real, if narrow (0.9â€“1.8 m), strip of paving that
...
`roadarea` fill, which sits at z=0. Painting more pavement can't fix this â€”
crossings are deliberately never painted as pavement (Â§2's own argument). The
...
presumably why this round left it alone three times running (Â§10, Â§14, Â§26)
â€” it should be the first change made once the file is free.

## 2026-08-24 â€” the five lanes merged, and the number went 795 m to 87 m (`acer/w-integrate`, PR #223 MERGED, branches deleted)

The integration pass. All five walk lanes â€” door, sidewalks, stairs, lit, ui â€”
...
extra metres over the pairs the router makes worse went **795.3 m â†’ 87.0 m**,
and the signed total â€” which the baseline warned was the real constraint,
because nine of nineteen pairs got LONGER when forced onto UT's single door â€”
went **+209.5 m â†’ âˆ’393.7 m**. So the merged router is net shorter across the
twenty, not just better on the bad ones. All 38 ends now land at the door UT
publishes; EERâ†’NHB, the baseline's worst at 569 m for a 271 m walk, routes at
272 m.

**The premise this pass was given was out of date.** It said the baseline meter
had never been built. It had â€” the baseline lane landed it on `main` as
...
`area == "yes"` branch, but they key on different `highway` values â€” a closed
`pedestrian` way is a mall whose rim is a walk, a closed `steps` way is a
terrace to be drawn as a polygon â€” so both arms are kept and `ground.geojson`
...
"Avoid stairs" checkbox was unusable â€” and measuring it three ways showed this
...
its self-check failed on 15 of 19 pairs with drifts to 27 m â€” every "extra
metres" it printed on those branches was uncalibrated. It reads both terms off
the page now; drift is 0.00 m on all 19.

**Judgement â€” and the brief for this pass was wrong about it, twice over.** It
...
- `w-ui` **06dd719, round 8: a genuine blind win.** Citymapper's own product
  ...
  drive), shuffled, and judged before the mapping was revealed â€” ours preferred
...
- `w-door` **bd73cc6, post-round-7 `oursWins=true`**, and `w-sidewalks`
  **da63da4, "it wins, verified independently"** â€” both independently
...
- `w-stairs` **bdff3f8, round 1 `oursWins=false`** (Citymapper bar unfetched) is
  ...
  builder; rounds 2â€“8 were never judged.
- `w-lit` â€” **no verdict at any round.**
...
390Ã—844 with touch and the walk UI not painted in any of them; plain page 0
console errors; OSM attribution visible in all five modes; and the detector is
not vacuous â€” under `?walk=1` it does report `wf-sheet` painted. Suite on the
merged result: `harness-drift` PASS, `movement` 14/14, `collision` 8/8,
`coplanar --gate` clean after an accepted baseline move (entrances 1627â†’1786 is
`w-door`'s 148 new doors; pairs-per-door went DOWN 1.147â†’1.140 and the
...
## 2026-08-24 â€” recon only, no code: what UT itself actually exports
...
room where the class meets. Buildings are abbreviated with three letters" â€”
...
`CMA 6.146`, `JGB 2.302` â€” code, space, room, nothing fancier. UT itself does
not appear to publish a native calendar/.ics export of a personal schedule
anywhere public â€” the actual registration system (`utdirect.utexas.edu`)
...
Sheet (your registration time and holds), not a schedule tool at all â€” the
...
really are Pickle Research Campus, about 11 km north of the map â€” confirmed
...
missing from UT's register the way the brief said â€” it's a real, registered
...
a bug in this app's own routing graph, not a missing UT record â€” a smaller
and different fix than the brief assumed.

## 2026-08-24 â€” recon only, no code touched: what Apple Calendar's own subscribe flow actually promises (ahead of the class-schedule-import feature)
...
Apple Calendar, UT's registration export). No real screenshot was possible â€”
...
shared with another concurrent task â€” so it was dropped and everything below
...
The useful finding for the seam Simeon named ("MAI 220, TTh 2:00pm" â†’ a
building CODE): both of Apple's two import paths â€” a live `webcal://`
subscription and a downloaded `.ics` file â€” end up as the same plain ICS
...
rewritten â€” which is exactly the shape Simeon asked for.
...
never itemize the choices for, not a push â€” so neither "subscribe" path
...
**2026-08-24, acer lane, `acer/si-gaps` â€” the eleven buildings the router had
...
The answer split them three orders of magnitude apart. Ten are 10.8â€“11.8 km
north at the Pickle Research Campus with no pavement within nine kilometres â€”
...
not a pavement problem and not a door problem â€” the code simply was not in any
...
**SSW now routes** (JES â†’ SSW, 660 m; PCL â†’ SSW, 755 m) through the same
mechanism that already made HLB work â€” walk to UT's own coordinate when we have
no door of our own. `walkmeter`'s "cannot route to at all" count went **11 â†’ 10**,
routable buildings scored 56 â†’ 57 with all 57 landing inside 15 m of UT's own
door, step-free reachability 56/56 â†’ 57/57, and the avoid-stairs door check
9/9 â†’ 10/10 clean. **All twenty baseline pairs are identical across all seventeen
measured fields â€” 340 comparisons, no differences** â€” and the live UI gate still
...
`notfound` went **11 â†’ 0**.

Nothing was re-baked. The obvious fix â€” add SSW to `entrances.geojson` and
re-run the bake â€” was tried and rolled back after measuring what it dragged in:
...
deliberately not fixed â€” a now-stale line in `walkmeter.mjs`, the missing copy
for an off-map building, the stale bake, and two buildings the search list greys
out even though they route â€” are written up with exact patches in
`docs/si-gaps.md`. Frames: `shots/si/gaps/`. `WAYFIND.on` untouched; the change
is additive only, five hunks, no deletions.

---

## 2026-08-24 â€” critic verdict on the schedule-import "UI" piece, round 3: `acer/si-ui` does not exist, oursWins = false
...
directly against GitHub, and `gh pr list --state all` â€” no branch, no open or
...
byte-identical to `main` (8,238 lines, no `.ics`/import code anywhere) â€” no
...
`playwright-core`/`chrome.mjs`, 390Ã—844, `?walk=1&drift=0`,
...
card visible in a real screenshot) â€” so the walking feature itself is intact
...
on our side to photograph â€” `shots/import/bar-google/` and
...
**oursWins = false.** Not "loses on comparison" â€” there is no comparison to
run. **The single biggest gap, concretely:** the schedule-import feature
described in the brief (three intake paths â€” Google Calendar, Apple Calendar,
UT registration export â€” feeding one `parseScheduleText(...)  â†’
...
build a first version â€” even the simplest version, a single "Import schedule"
button opening a paste-box that runs an `.ics`/text parser against the
existing `UT_ENTRANCES`/`UT_CELEBRATED` code table â€” before there is anything
...
at Pickle Research Campus â€” already confirmed in the recon entry above against
...
missing from UT's register â€” same conclusion as the recon entry, re-confirmed
...
bug, not a missing building record â€” did not chase that bug down further,
...
`scripts/verify` â€” the one written to drive this check was deleted after use.
...
## 2026-08-24 â€” critic round 4 verdict on `acer/si-parser`: oursWins = true, one real gap found in the auto-sniff path
---
...
`?walk=1` page with `playwright-core` and real Chrome â€” not the builder's own
screenshots.
...
branch â€” 209/209 assertions, independently reproduced.

**Then built four fixtures of my own from scratch** â€” different building
...
answer next to a decoy course-shaped token â€” to make sure I wasn't grading the
...
code and a suggestion where one existed (`WEK`â†’`WEL`, `CBQ`â†’`CBA`), and the
...
per house rule â€” both really do show a drawn card and route (GDCâ†’Welch,
...
literal substrings `BEGIN:VCALENDAR` / `BEGIN:VEVENT`. An HTML sign-in page â€”
`docs/si-parser.md`'s own `not-a-calendar.ics` fixture, modelled on the UT EID
wall â€” contains neither, so it auto-detects as `'rows'` and is fed to
...
a course but no building` â€” a wrong, actively misleading sentence (there is no
...
only reachable if the caller manually forces `kind:'ics'` â€” which is exactly
...
hint â€” which is more than Google's bar gives a student on any bad file, ever.
...
substrings, so any non-calendar text that lacks those literal tokens â€”
starting with the HTML sign-in page the builder's own docs name as the most
likely real bad file â€” is silently routed into `schedParseRows` and reported
...
additive-only claim. Branch left as found â€” deleted the four scratch `.mjs`
...
I re-ran it (same camera, same fixture â€” not a real change, just noise from
running their own script). Server on 8951 killed, port re-confirmed free by
`netstat`. No files the builder owns were edited.

## 2026-08-24 â€” critic round 3 on the privacy piece (`acer/si-privacy`), oursWins=true
---

## 2026-08-24 — critic round 3 on the privacy piece (`acer/si-privacy`), oursWins=true

Drove `acer/si-privacy` for real on port 8955, own browser, own scripts â€” not the
...
under `?walk=1&drift=0&intro=0`, routed GDCâ†’PAR to generate real traffic, and
scanned all 153 captured requests (page + worker fetches) â€” zero carried any
...
fresh document â€” no `austin3d.schedule.*` key in local or session storage, no
reserved IndexedDB database, panel back to "No schedule saved on this device
yet." With `?walk=1` absent, `window.fetch` is the untouched native function â€”
...
2 classes and the exact source label passed in, empty state after reload) â€”
...
The one real problem: `docs/si-privacy.md` Â§7 backs its central factual claim â€”
"SSW... demolished September 2024; the school moved to Walter Webb Hall" â€” with
...
9.6â€“10.6 KM for the ten real Pickle buildings) and concludes the opposite: SSW
is a real, current, main-campus building UT surveys doors for, missing only a
row in our own register â€” a one-line fix, not a demolition. `si-privacy`'s
...
handed), so it doesn't break THE BAR â€” but it's a fabricated source backing a
...
drop the demolished/moved narrative and the fake citation from Â§7, or replace
...
## 2026-08-24 â€” critic round 4 on the schedule-import UI piece (`acer/si-ui`), oursWins=true, but the bar in the repo is fake
...
`window.wayfindSearch` after forcing the graph to load â€” 12 came back
...
`[]` â€” not found at all, meaning the general search box treats "SSW" as an
...
of the live lookup â€” so the message a student sees is correct regardless â€”
...
myself: 450 m / 5â€“7 min unchanged, and `walk=0` still drops zero DOM nodes,
...
`shots/import/bar-apple/` â€” the exact paths the brief named for the real
products â€” hold nothing but this app's own panel, captioned in `docs/si-ui.md`
as "Google Calendar â€” the add screen" and "Apple Calendar â€” a subscription
...
wall â€” expected, not attempted further. What worked was searching out
...
`howtogeek.com`'s 2019 Apple Calendar walkthrough (the actual File â†’ New
...
you already know File â†’ New Calendar Subscription exists, while ours tells
...
that pair isn't a fair fight â€” Apple's product has no per-event validation to
...
files with real product captures â€” my downloaded copies
...
Calendar walkthrough) are a ready starting point â€” or rewrite the captions to
...
No scratch scripts or screenshots committed to the repo â€” all work stayed in
the scratchpad.

---

## 2026-08-24 â€” critic verdict on the "dayview" piece, round 3: oursWins = true
...
scripts/verify/dayview.mjs 8954` â€” **59 ok, 0 failed**, including a live re-probe
of all eleven forcing-function codes (matches the doc: ten really are 10.8â€“11.8
...
UI gate PASS â€” identical to the figures quoted in `docs/si-dayview.md`, so the
...
day-plan section myself â€” the only hit is a code comment citing the forbidden
phrase as an example of what NOT to print.

**Blind visual judgement.** Bar = a real, current Google Calendar product
screenshot (workspace.google.com's own Calendar marketing page, fetched live â€”
...
the stated question â€” does it read at a glance which walk is next, how long it
takes, and whether it has a problem â€” before un-shuffling. Verdict: **ours,
clearly.** Calendar's day view has no visual language for the gap *between*
events at all â€” no duration number, no problem flag, nothing but blank space â€”
...
`13â€“18 min Â· 1.1 km` numbers, a gap bar sized to the schedule's own passing
period with a hatched stub where the walk runs over it, and one-line chips
(`Tight for this gap`, `1 set of stairs Â· a step-free way is 29 m shorter`,
...
(`gaps-desktop.jpg`, `tth-done.jpg`, `clip-nothing.jpg`, both phone shots) â€”
...
~1,000-line section to `js/wayfind.js` at the identical insertion point â€” right
after the same closing brace at the end of the file â€” so combining them is not
a clean fast-forward. Confirmed with `git merge-tree 80747c4 origin/acer/si-dayview
origin/acer/si-ui`: a real conflict in `js/wayfind.js` (dayview's Â§10 vs. ui's
Â§9, both wanting to be the first new section after the existing code), plus the
same shape of conflict against `origin/acer/si-parser`, and between si-ui and
si-parser too â€” all three pairs conflict. Checked for the worse, silent version
...
nowhere in si-dayview's â€” so the fix is mechanical (concatenate both blocks,
...
committed â€” all frames stayed in the scratchpad; the five already-committed
`shots/si/dayview/*.jpg` from the builder's own harness run were reused rather
than re-shot.

## 2026-08-24 â€” critic round 5 verdict on `acer/si-parser`: oursWins = true, the round-4 bug is actually fixed
...
that bug, so the job was to find out whether it actually holds â€” not to take
...
same as the main checkout â€” `npm install` was needed), served the branch on
...
**Then built my own fixtures from scratch â€” different building codes, wording,
and even a different HTML shape than the builder's or round 4's â€” because a
...
- A four-event bad file covering the brief's four named categories in one
  ...
  `js/wayfind.js:3627`) rather than erroring â€” the multi-line-address case is
  a success path, not a failure one, and it worked.
- A same-day back-to-back pair (`PAR`â†’`SZB`, codes nobody else has used)
  produced three real legs (MO/WE/FR) through `wayfindScheduleCheck`, each
  `ok:true` at 451.74 m â€” checked against a direct `wayfindRoute('PAR','SZB')`
  call, which returned the identical 451.74 m. Not "marked routable" â€” an
  actual route, matching to the centimetre.
- My own saved-sign-in-page fixture â€” a UT **CAS** login shell, deliberately a
  different markup shape than the builder's UT-EID-styled fixture, to check
  the fix generalises rather than being tuned to one exact file â€” correctly
...
- My own "one real class buried in unrelated boilerplate" paste (registration
  ...
  (`JGB`, GEO 401) while flagging the four junk lines individually â€” the
...
1,448 pixels moved vs a bar of 21,340 â€” fail. Independent run 2, same
fixtures, same machine, immediately after: 472 pixels moved vs a bar of
1,160 â€” fail again, but with entirely different numbers. The doc's own
...
the same scene â€” the city's own ambient animation (the day/night dial, or
...
claim â€” the leg is really drawn â€” is true. The instrument measuring it is not
...
of y events"** â€” a bare count, no per-event detail, "does not provide
...
reads `1351  0` â€” confirmed additive-only myself, not quoted from the
...
(`JES`â†’`WEL`) still returns 449.61 m through the live page, matching prior
rounds' figures â€” the shared file's core routing is intact. The eleven
...
rather than a wrong or generic one â€” the parser's own resolver already knows
...
varied 0 / 116 / 2,134 pixels across three runs on the same machine â€” twice
...
per CLAUDE.md's own "take the minimum of interleaved reps" rule â€” right now a
...
additive-only claim; a same-machine sanity route (`JES`â†’`WEL`); and the
`SSW`/`HLB`/`MER` claims reproduced by calling the functions myself. Branch
left as found â€” the two regenerated screenshots were reverted to the
...
## 2026-08-24 â€” critic round 3 on the gaps piece (`acer/si-gaps`, tip `7fd3596`), oursWins=true

Fresh context, own scripts, own server on port 8952 (`scripts/verify/node_modules`
was empty in this worktree â€” `npm install`-ed fresh, then the two files it
...
lines diffed byte-for-byte identical between the two runs (`diff` exit 0) â€”
...
`SSW Â· School of Social Work Building Â· 2 doors`, class `wf-item active` â€” not
...
`7-11 min walk Â· 660 m Â· No stairs on this route`. Screenshotted twice and
read both with the Read tool: the closed card, and the map after clicking
`Show route` â€” a real cream route line following an actual footpath past DKR
...
naive `diff` claim the whole file changed): 307 insertions, 0 deletions â€”
additive-only holds.

**No sibling-lane regression risk right now, checked rather than assumed:**
`git merge-base origin/main origin/acer/si-gaps` equals `origin/main` HEAD â€”
...
all). It left untouched the larger, harder class the doc's own Â§6(d) already
found and sized: **14 `nodoor` codes are real, drawn, on-map buildings whose
nearest mapped door belongs to a different building** â€” SSW-shaped, but
...
and was correctly not attempted blind â€” the doc measured that a clean re-bake
...
## 2026-08-24 â€” critic verdict on the "dayview" piece, round 4: oursWins = true, and a real defect the harness cannot see
...
hardcoded demo fixtures for that part â€” reproduced the round-4 builder's scratch
...
the `MAII`â†’`MAI` typo refusal, the done-state, off-stays-off).

**Then, separately from the builder's own fixtures, fed the parser lane's real
`ut-regplus.ics`** â€” a genuine UT Registration Plus export, not written by
either lane's harness â€” through the real running pipeline
(`window.wayfindParseSchedule` â†’ `window.wayfindDayFromSchedule`), no shortcuts.
It resolves to a completely ordinary Tuesday: 3 classes (RTF 305, J 310F, C S
429), 2 walks, 1.5 km on foot â€” exactly the "realistic 3-4-class day" the brief
...
**Blind visual judgement.** Bar = a real, current Google Calendar screenshot â€”
...
shuffled, judged on the stated question â€” does it read at a glance which walk is
next, how long it takes, and whether it has a problem â€” before revealing the
...
(`1 set of stairs Â· a step-free way is 180 m shorter`, `Crosses 3 signalised
crossings`) rather than something to infer; neutral-2 is a well-designed list of
appointments with zero visual language for travel between them â€” the gap
...
real bar image, so this isn't one lucky comparison â€” the day-view's core claim
...
the blind test also showed the day's third class (`C S 429`, 4:00â€“5:00pm,
University Teaching Center) silently truncated mid-line, with no scrollbar, no
fade, no "more below" cue of any kind â€” the row is just cut, mid-glyph, and the
...
`scrollHeight` (398px) exceeds its `clientHeight` (348px) at 390Ã—844, the row is
...
any kind â€” grepped the file for `mask-image`, `fade`, `gradient.*transparent`
...
the OUTER panel's bounding box against the viewport â€” which is true, the panel
itself ends at y=658 of 844 â€” and never checks whether every row inside the
internal scroll region is reachable or hinted at.** That is a real, ordinary UT
schedule â€” three classes is not an edge case â€” silently losing its last class on
...
asking a question:** on a 390Ã—844 phone, `#wf-day-list` needs a visible
affordance that content continues past its `max-height:66vh` clip â€” a bottom
...
element exists when it doesn't) for a 3+ class day at phone width â€” the current
...
`wayfindDayFromSchedule` pipeline at both 1280Ã—800 and 390Ã—844; the `#wf-day`
...
Calendar surfaces. It is not a tie and not a squeak â€” Calendar structurally
...
the repo â€” all frames stayed in the scratchpad.

## 2026-08-24 â€” critic round 5 on the privacy piece (`acer/si-privacy`), oursWins=false â€” round 5's fix closed one shape of the hole and left an adjacent one wide open

Fresh context, port 8955, own script (not the builder's `r5-blob.mjs`) â€” own
...
(`window.WAYFIND.store.save()` â€” the same call a real importer will make, since
...
reload of the seeding page, so the assertion cannot be testing its own seed) â€”
...
`opaque` flag) and the fix genuinely holds for `Blob` and `ReadableStream` â€” I
did not need to re-check that, the reasoning in `js/wayfind.js` Â§12 and
`docs/si-privacy.md` Â§6 is sound and I read the real code, not just the doc.
...
with **no `opaque` flag raised** â€” unlike a `Blob`, which now sets
...
1. `postMessage({ tile:'x', payload: encoder.encode(canaryTitle).buffer },
   [buffer])` into a worker â€” the same transfer shape MapLibre uses for tile
   bytes. It went through. `guard.state()` before/after: `blockedDelta: 0`,
   **`opaqueDelta: 0`** â€” not merely unblocked, *uncounted, unlogged, invisible
   to the guard's own instrumentation.* The worker received the schedule title
   intact (`byteLength: 30`, decodable).
2. From inside that same worker, an ordinary `fetch()` â€” the worker's own
   global `fetch`, which `installEgressGuard()` never touches because it only
   patches `window.fetch` on the **main thread** â€” POSTed those bytes to my raw
...
uses routinely. The doc's safety-case sentence â€” *"a worker cannot send what it
was never told"* â€” is false as written: a worker can be told, via a typed
array, without the guard so much as noticing, and once told, nothing in this
file stands between that worker and the network at all. Â§9's "what it does not
...
those â€” it is the exact transport shape the app's own tile pipeline already
...
does not get to tie on a technicality of the stated test â€” the mechanism it
leans on is still concretely breakable, three rounds in.

**Single biggest remaining gap, concretely:** in `scanStructured()`
(`js/wayfind.js` Â§12, the walk used by the `Worker.prototype.postMessage`
...
treats `Blob` and `ReadableStream` â€” do not just skip past it. Then, because
...
say so explicitly in Â§9's "what it does not cover" list instead of asserting
in prose that "a worker cannot send what it was never told" â€” because right
...
scratchpad, not committed â€” nothing here cites it as evidence, so per CLAUDE.md
rule 12 it does not belong in the repo.
---

**2026-08-24, acer lane, `acer/si-gaps` round 3 â€” the building a schedule names,
...
inheriting it â€” the eleven, the eleven-kilometre distances, and the fact that
UT's two SSW doors land within half a metre of a building this app already draws
â€” and then fixed the one thing round 2 had written off as somebody else's job.
...
Social Work Building â€” 2 doors* and opens. Same for the Health Learning
...
this build cannot reach at all goes 11 â†’ 10. The ten that remain are at another
campus and are answered as such instead of in silence. `data/walk_graph.json`
and `data/entrances.geojson` deliberately untouched â€” the graph bake is eight
