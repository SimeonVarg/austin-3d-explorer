/**
 * places-shot.mjs — shot.mjs, but with the URL query under my control.
 *
 * Exists for exactly one reason: the BEFORE half of a before/after pair. The
 * only honest "before" for this pass is `?places=0`, which makes js/places.js
 * return before it adds a source or a layer — so the same build, the same
 * camera, the same hour and the same everything-else render without the
 * storefronts. Setting the layers to `visibility: none` instead would look
 * identical here and would NOT be equivalent in general: it leaves the source
 * fetched, tiled and in memory, which is most of what the pass costs, so the
 * same lever has to serve places-perf.mjs and it has to be a genuine off.
 *
 * Derived from drag-shot.mjs, which learned all of the waiting below the hard
 * way. Copied rather than imported because that file belongs to another pass.
 *
 * Usage: node places-shot.mjs <outPrefix> <shots.json> [extraQuery]
 *   node places-shot.mjs before shots-places.json 'places=0'
 *   node places-shot.mjs after  shots-places.json
 */
/*
...
const OUT = process.argv[2] || 'places';
...
async function openSession() {
  ...
  const hasPlaces = await page.evaluate(() => !!window.__map.getLayer('places-solid'));
  console.log('session up |', URL, '| places-solid present:', hasPlaces);
