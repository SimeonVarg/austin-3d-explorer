...
r = await at(14.0, 4000);
check('the slider maximum means unlimited, not a 4 km cull',
  r.fine.shown === r.fine.present && r.mid.shown === r.mid.present,
  `alt ${r.altActual} m, D=4000 — fine ${r.fine.shown}/${r.fine.present}, mid ${r.mid.shown}/${r.mid.present}`);
...
const foreign = await page.evaluate(async () => {
  ...
  window.GFX.renderDistance = 4000;                            // unlimited: lod wants everything shown
  window.applyLOD(m);
