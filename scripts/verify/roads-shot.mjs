/**
 * roads-shot.mjs — shot.mjs, but it also waits for `austin-roads`.
 *
 * A verbatim copy except for that one source name. shot.mjs is shared with five
 * other passes and adding a source to its wait list would be a conflict for all
 * of them; roads.geojson is 2.5 MB and does NOT finish inside shot.mjs's fixed
 * 4 s settle, so a Speedway frame taken through shot.mjs comes back with no
 * roads at all and reads as a broken layer.
 *
 * Usage: node roads-shot.mjs <outPrefix> [shotsJson]
