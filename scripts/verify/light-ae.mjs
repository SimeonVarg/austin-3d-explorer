...
// ── The authored poses: gain must sit near 1.0 (protect the look) ────
// The metering knee makes this exact: deviations inside ~±17% of the hour's
// target are the authored look and produce NO correction.
...
// ── Deviations must move the gain the right way ──────────────────────
// Settled poses across the whole day cluster within ~±10% of their hour's
// typical (measured: day 0.49-0.56, golden 0.49-0.50, night 0.135-0.136) —
// flat-lit extrusions have no sustained bright outlier. The real dark outlier
// is the deep-night sky away from the moon (~0.086). For the pull-DOWN
// direction, make a genuinely bright RENDERED frame the honest way: whiten
// the ground layers (the README's "paint it magenta" trick) and meter that.
